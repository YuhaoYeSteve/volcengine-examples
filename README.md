...
- **v3.0.2** (2026-02-06)
  - 🗑️ 简化：移除设置中的 Model Endpoint ID 输入框，统一使用后端配置，减少用户配置负担。

- **v3.0.1** (2026-02-06)
  - 🐛 修复：后端服务重启以正确支持图片上传的数据结构。
  - ⚡️ 优化：前端增加错误消息的气泡提示，避免请求失败时无响应。

- **v3.0.0** (2026-02-05)
  - 🎨 极致还原：深度复刻豆包 (Doubao) 网页版 UI，从配色到布局完全重构。
  - 🖥️ 侧边栏：移除突兀的蓝色按钮，采用极简灰白风格，增加“AI 搜索/创作”入口占位。
  - 💬 对话体验：
    - 用户气泡改为清新的淡蓝色 (`#EBF5FF`)，文字深色高对比度。
    - AI 回复去除气泡背景，采用文档式沉浸排版，更适合阅读长文。
  - ⌨️ 输入增强：悬浮式输入框，底部集成功能工具栏（深度思考、联网搜索等），视觉效果对标原生应用。

- **v2.2.2** (2026-02-05)
  - 💎 UI 细节优化：
    - 调整主色调为更通透的琉璃蓝 (Blue-500/80)。
    - 为按钮和气泡增加细腻的玻璃边框和模糊效果。
    - 优化“新建对话”按钮和发送按钮的视觉一致性。
